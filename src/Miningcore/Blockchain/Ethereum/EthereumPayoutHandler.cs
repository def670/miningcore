...
public class EthereumPayoutHandler : PayoutHandlerBase, IPayoutHandler
{
    ...
    private async Task<string> PayoutAsync(Balance balance, CancellationToken ct)
    {
        ...
        if (extraPoolConfig?.ChainTypeOverride == "Ethereum" || extraPoolConfig?.ChainTypeOverride == "Ubiq" || (extraPoolConfig?.ChainTypeOverride == "EtherOne") || extraPoolConfig?.ChainTypeOverride == "MainPow" || extraPoolConfig?.ChainTypeOverride1 == "EtherOne")
        {
